---
title: "Survey II Final Project"
author: "Isabel Monge, Phong Duong & Marvin Struckmeyer"
output: 
  html_document:
    toc: true
    theme: cerulean
...
-   z-score standardization: for continuous variables to make them
    comparable (mean=0, sd=1)
-   min-max normalization: for variables with natural bounds (e.g.,
    0-100 scores)
...
We can see that non-response in qc19 is not random; it is highly
correlated to numerous variables (both categorical and continuous) such
as countries (various q1) or regions (p7) responses to LGBT/
discrimination questions (such as qc6, qc7, qc12, qc13 and qc18) and
paradata/ survey cooperation (p5). Therefore, the 12% missingness of
qc19 is definitely not MCAR, but at least MAR or even MNAR.

## Data cleaning

"Don't know" (DK) responses are especially problematic for ordinal
variables, because leaving them as is would inflate support levels,
while recoding them as 0 could misrepresent them as the most negative
response. We will therefore code all DKs and refusals (and a few other
special cases) as NA.
...
We also needed to make decisions about whether or not to impute missing
variables from a few of our external datasets. We chose to delete the
variables from the ESS because the Eurobarometer survey already contains
similar information and imputing these values didn't seem appropriate.
However, we did choose to impute the happiness score using KNN.
...
We will use this reduced dataset when building our explanatory model. We
chose these variables based on previous literature on transgender rights
and the results of our descriptive analysis.
...
# function to compute and plot top correlations
plot_top_correlations <- function(data, vars_block, outcome = "qc19", block_title = "Block") {
  needed_vars <- intersect(vars_block, names(data))
  df_block <- data[, c(outcome, needed_vars), drop = FALSE]
  if (!is.numeric(df_block[[outcome]])) {
    df_block[[outcome]] <- as.numeric(as.character(df_block[[outcome]]))
  }
  df_block_num <- df_block %>% select(where(is.numeric))
  if (!(outcome %in% names(df_block_num))) {
    message("Outcome variable not numeric in ", block_title)
    return(NULL)
  }
  cor_df <- data.frame(
    variable = setdiff(names(df_block_num), outcome),
    correlation = sapply(setdiff(names(df_block_num), outcome), function(var) {
      cor(df_block_num[[var]], df_block_num[[outcome]], use = "complete.obs")
    })
  )

  # sort by absolute correlation for readability & keep top 15
  cor_df <- cor_df %>% arrange(desc(abs(correlation))) %>% head(15)
  ...
  ggplot(cor_df, aes(x = reorder(variable, correlation), y = correlation)) +
    ...
    coord_flip() +
    labs(
      title = paste0("Top Correlated Variables with ", outcome),
      subtitle = paste0(block_title, " - Aggregated Data"),
      x = "Variables",
      y = paste("Correlation with", outcome)
    ) +
    theme_minimal() +
...
# generate correlation plots for each block
plot_top_correlations(df_reduced, block_demographics, "qc19", "DEMOGRAPHICS")
plot_top_correlations(df_reduced, block_personal_identity, "qc19", "PERSONAL IDENTITY")
plot_top_correlations(df_reduced, block_discrimination, "qc19", "DISCRIMINATION")
plot_top_correlations(df_reduced, block_paradata, "qc19", "PARADATA")

# ### Random forest for individual variables
...
Based on the results of our descriptive analysis, we should use the
following variables in the multilevel regression model: z_rainbow_score,
z_gender, regions, z_v2x_libdem, z_v2z_egaldem,
z_functioning_of_government and norm_gdp_2018. First, note that we found
that z_functioning_of_government is also highly positively correlated to
to transgender support (we did not show it here because of problems when
adding together the individual pieces of code). Second, note that while
region seems to be highly related to transgender support, we decided not
to include it. First, region is descriptive rather than explanatory.
Second, including it would create a three-level multi-level structure
rather than a two-level strucutre.
...
We see similar variables are most important using descriptive ML,
reinforcing our previous decision.
...
The null model indicates a statistically significant baseline
probability of support for transgender individuals obtaining official
documents (intercept = 0.45, p = 0.015), with substantial country-level
variation accounting for 22.7% of the total variance in this support
(ICC = 0.227).
...
Model 1 significantly improves fit over the null model (χ² = 1720.5, p
\< 0.001, lower AIC/BIC). Transgender documentation is positively
associated with characteristics such as being female and having
LGB/transgender friends, while negatively associated with age and
respondent cooperation during the survey.
...
Model 2 improves fit yet again by adding country-level predictors (χ² =
33.7, p \< 0.001, AIC reduced by 29.7 points). The strongest predictors
are LGB friends (0.88), egalitarian democracy (0.49), transgender
friends (0.42), and LGBTQ legal protections (rainbow score, 0.37), while
individual-level predictors remained consistent with Model 1.
...
Model 3 slightly improves fit by adding random slopes for LGB friends
(χ² = 7.92, p = 0.019). There is modest cross-country variation in the
effect of having LGB friends (SD = 0.176), with a negative correlation
(-0.48) between intercepts and slopes indicating stronger effects in
countries with lower baseline support.
...
Testing cross-level interactions shows that neither the interaction
between LGB friends and Rainbow score (p = 0.90) nor between LGB friends
and egalitarian democracy (p = 0.41) is significant. Adding these
interaction terms actually worsens model fit (AIC increases).
...
Model 3 (random slopes for LGB friends) is the best model, as it
significantly improves fit over simpler models (χ² = 7.92, p = 0.019)
and achieves the lowest AIC (26472.5). Subsequent interaction models
fail to improve fit (p = 0.90, p = 0.41) and slightly increase AIC,
confirming Model 3 as the most robust choice.
...
```{r}
# Reading data set that was already prepared for the ML training
data <- read_rds("R/data_for_ML.rds")
...
train %>% 
  ...
  ungroup() %>% 
  ggplot(aes(x= qc19, y=percent, label=count, fill = qc19)) +
  geom_col() +
  geom_text(vjust = -0.3) +
  scale_y_continuous(labels=scales::percent) +
  theme_minimal() +
  labs(y = "Percentage", x = "qc19",
       title = "Mild Imbalance of Data (60%-40%) in qc19 Categories") +
  theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
        legend.position = "none")
```

There is some degree of imbalance in the data for our targeted variable.
We will apply all 3 balancing techniques (Undersampling, Oversampling,
and Synthetic Data) during resampling.
...
train_enet <- read_rds("R/regularization/enet_training_data.rds")
```

As the regularization takes a long time to load, we saved dataset with
already selected features after Elastic Net feature reduction so it is
readily available.
...
Similarly, these models take a long time to train. Therefore, we will
only load the pre-trained models using this code.

### Performance Evaluation

\*Note: due to time constraint, we were only able to train 7 models.

```{r}
# Performance on training data

# Random Forest
rf_under <- read_rds("R/models/rf_enet_under.rds")
#rf_over <- read_rds("R/models/rf_enet_upper.rds")
rf_smote <- read_rds("R/models/rf_enet_smote.rds")

# Gradient Boosting Machines
gbm_under <- read_rds("R/models/gbm_enet_under.rds")
gbm_over <- read_rds("R/models/gbm_enet_upper.rds")
gbm_smote <- read_rds("R/models/gbm_enet_smote.rds")


# Support Vector Machine
svm_under <- read_rds("R/models/svm_enet_under.rds")
svm_over <- read_rds("R/models/svm_enet_upper.rds")
# svm_smote <- read_rds("R/models/svm_enet_smote.rds")

#-------------------------------------------------------------------
# Summarizing performance of all models in training data
...
Random Forest model trained with synthetic data generation seems to be
the best-performing model on training dataset. We will evaluate again
the performance of the models on testing set.

```{r}
# Prediction on individual level
...
#-------------------------------------------------------------------
# Create a dataframe of the model's performance
...
The Random Forest with synthetic data balancing performs the best on the
testing, even with slightly better performance than in the training set.
We, therefore, will use it as the model for prediction.

```{r}
# Prediction on country level

country_comp <- test            # replicate test set
country_comp$pred <- p_rf_s    # create new column of predictions from Random Forest (using synthetic data generation)

#------------------------------------------------------------------------
# Summarizing real support level of countries 
country_original <- country_comp %>%  
  group_by(isocntry, qc19) %>%
  summarise(count = n()) %>% 
  group_by(isocntry) %>% 
  mutate(pcg = count/sum(count))
# Summarizing predicted support level of countries 
country_pred <- country_comp %>% 
  group_by(isocntry, pred) %>%
  summarise(count = n()) %>% 
  group_by(isocntry) %>% 
  mutate(pcg = count/sum(count))
# Joining the previous 2 summarized dataframes
comparison <- cbind(select(country_original, c(isocntry,qc19, pcg)), country_pred$pcg) %>% 
  ...
  mutate(type = factor(type, labels = c("Original Responses", "Predicted Responses")))

#-----------------------------------------------------------------------------
# Visualization of both predicted and original support levels among countries
ggplot(comparison, aes(x= reorder(isocntry, value), y=value, fill=response)) +
  ...
  labs(x = "Country", y = "Support Level",
       title = "Predicted Responses (RF) Displays Similar Results to Real Responses") + 
  theme(legend.position = "none",
        plot.title = element_text(hjust = 0.5, face = "bold", size = 14))
```

We can see that on a country level, the predictions from our models are
quite accurate and reflects well the support level of the real dataset.

### Filling The Gap

```{r}
# Loading original full dataset (without removing DK answers)
qc19_uniterated <- read_rds("df_rf_enriched.rds") %>% select(qc19) %>% pull()
full_data <- read_rds("df_rf_enriched_new.rds") %>% 
  select(-1) 
full_data$qc19 <- qc19_uniterated  

full_pred <- predict(rf_smote, full_data) # Prediction using RF synthetic data
full_data$pred <- full_pred                # Create new column of the prediction

# Replace DK answers with predicted data
full_data <- full_data %>% 
  mutate(qc19_enhanced = ifelse(qc19 == 3, pred, qc19))

#-----------------------------------------------------------------------------
# (Enhanced) Summarize data by country
sum_enhanced <- full_data %>% 
  group_by(isocntry, qc19_enhanced) %>% 
  summarise(count = n()) %>% 
  group_by(isocntry) %>% 
  mutate(pcg = count/sum(count))
# (Enhanced) Ranking of countries list
sort_enhanced <- sum_enhanced%>%
  filter(qc19_enhanced == 1) %>% 
  arrange(desc(pcg)) %>% 
  pull(isocntry)

# (Original) Summarize data by country
sum_origin <- full_data %>% 
  group_by(isocntry, qc19) %>% 
  summarise(count = n()) %>% 
  group_by(isocntry) %>% 
  mutate(pcg = count/sum(count))
# (Original) Ranking of countries list
sort_origin <- sum_origin%>%
  filter(qc19 == 1) %>% 
  arrange(desc(pcg)) %>% 
  pull(isocntry)

#-----------------------------------------------------------------------------
# Visualization of support level with predicted answers of DK responses
g_enhanced <- ggplot(sum_enhanced, 
                     aes(x=factor(isocntry, sort_enhanced), y=count, fill = factor(qc19_enhanced, levels = c("2", "1")))) +
  geom_bar(position="fill", stat="identity") +
    theme_minimal() +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual( values = c("indianred", "deepskyblue4")) +
  labs(x = "Country", y = "Support Level",
...
g_origin <- ggplot(sum_origin, 
                     aes(x=factor(isocntry, sort_origin), y=count, fill = factor(qc19, levels = c("3","2", "1")))) +
  geom_bar(position="fill", stat="identity") +
    theme_minimal() +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual( values = c("lightgrey", "indianred", "deepskyblue4")) +
  labs(x = "", y = "Support Level",
...
library(ggpubr)
ggarrange(g_origin, g_enhanced,
                  nrow = 2) %>% 
  annotate_figure(top = text_grob("Comparison of Enhanced Vs. Original Responses", face = "bold", size = 16, hjust=0.5),
                  left = text_grob("Support Level", rot = 90),
                  bottom = text_grob("Country"))
```

With the trained model, we filled in the surveys that answered "DK" and
estimated a better picture of the support level of all the countries.
The ranking are close to original ranking with some little movement of
positions. Spain is estimated to have the highest level of support
instead of the Netherlands while Czech Republic is expected to have less
support overall if all respondents gave a "Yes" or "No" answer to
question qc19.
